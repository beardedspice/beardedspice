...
BSStrategy = {
  ...
  toggle: function toggle () {return document.querySelectorAll('.playControl')[0].click()},
  next: function next () {return document.querySelectorAll('.skipControl__next')[0].click()},
  favorite:function () {
    var play = document.querySelector('.playControl');
    if(play.classList.contains('playing')) { play.click(); }
  },
  previous: function previous () {return document.querySelectorAll('.skipControl__previous')[0].click()},
  pause: function pause (){return document.querySelector('div.playControls button.playbackSoundBadge__like').click()},
